...
$MODE_EEDOMUS = 'eedomus'; // Dans ce mode, on maintient la temp�rature de consigne
...
function sdk_netatmo_html($extension_module){
	...
	if(sizeof($extension_module) > 0){
		$ret .= '<br>';
		$ret .= '<br>';
		$ret .= "Voici la liste de vos maisons et leurs pi�ces associ�es :";
		$ret .= '<br>';
		$ret .= '<ul>';
		//homes informations
...
if (strlen($prev_code) > 1 && $code == $prev_code){
	// on reprend le dernier refresh_token seulement s'il correspond au m�me code
	$refresh_token = loadVariable('refresh_token');
	$expire_time = loadVariable('expire_time');
	// s'il n'a pas expir�, on peut reprendre l'access_token
  	if (time() < $expire_time){
    	$access_token = loadVariable('access_token');
  	}
}

// on a d�j� un token d'acc�s non expir� pour le code demand�e
if ($access_token == ''){
	if (strlen($refresh_token) > 1){
		...
		// 1�re utilisation apr�s obtention du code
...
##### information lors de la r�cuperation du code oauth #####
if ($_GET['mode'] == 'verify'){
	...
	echo '<br>';
	echo "Vous pouvez copier/coller ces informations dans le param�trage de votre p�riph�rique Eedomus.";
...
// Les crit�res sont r�unis pour demander d'effectuer une action
if ($is_action){
	...
	// On va passer en mode manuel et forcer une temp�rature
	if($setpoint_temp != ''){
		$mode;
	...
	// On va seulement changer de mode sans modifier la temp�rature
...
// On effectue une lecture des donn�es
else{
	//variables
	$modules_name = array();
	$rooms_name = array();
	...
	if (isset($setpoint_temperature) && $setpoint_mode == 'manual'){
		...
		// Verification: besoin de l'imposer � nouveau ?
